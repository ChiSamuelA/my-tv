...
import { headerPolicyForStream, type UpstreamHeaderPolicy } from "./header-policy";
import type { PlaybackCapability } from "./types";

export interface CatalogBoundResource { upstreamUrl: string; headerPolicy: UpstreamHeaderPolicy }
...
export async function resolveCatalogBoundResource(
  ...
  return {
    upstreamUrl: value.resourceKind === "root-manifest" ? stream.url : value.resourceUrl!,
    headerPolicy: headerPolicyForStream(stream),
  };
}
